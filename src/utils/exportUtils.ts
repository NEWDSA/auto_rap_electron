  ...
  static async exportData(data: any[], options: Partial<ExportOptions> = {}) {
    ...
    try {
      ...
      // 使用自动保存模式，且有有效的预设路径
        ...
        const fullPath = `${savePath}${pathSeparator}${fileName}.${defaultExtension}`
      ...
      if (filePath) {
        ...
        if (typeof window !== 'undefined' && window.document) {
          if (output instanceof Blob) {
            ...
            // 发送到主进程进行保存
            await window.electronAPI.invoke('fs:writeFile', filePath, arrayBuffer, {
              encoding: 'binary',
            })
          } else {
  ...
  static async exportToText(data: any[], options: ExportOptions, filePath?: string) {
    try {
      ...
      if (filePath) {
        ...
        // 文本内容直接传递，无需转换为Buffer
        await window.electronAPI.invoke('fs:writeFile', filePath, content, {
