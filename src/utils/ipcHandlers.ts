...
export function initializeIpcHandlers() {
  ...
  if (typeof window === 'undefined' || !window.electronAPI) {
    console.info('IPC处理程序未初始化：当前环境不支持 electronAPI')
    return
  }
  ...
  window.electronAPI.on(
    ...
    async (event: IpcRendererEvent, message: ExportMessage) => {
      try {
        ...
        console.log(
          `准备导出数据: 类型=${options.type}, 文件名=${options.fileName}, 保存模式=${options.saveMode}`
        )
