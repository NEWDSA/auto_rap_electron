...
export class CaptchaService {
   ...
   */
  async recognize(imageData: string | Buffer, captchaType: string = 'normal'): Promise<CaptchaResult> {
    try {
   ...
   * 百度OCR识别
   */
  private async recognizeWithBaidu(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
   ...
   * 腾讯OCR识别
   */
  private async recognizeWithTencent(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
    // 腾讯云OCR API实现
   ...
   * 阿里云OCR识别
   */
  private async recognizeWithAliyun(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
    // 阿里云OCR API实现
   ...
   * 有道智云OCR识别
   */
  private async recognizeWithYoudao(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
    ...
    const text = result.Result?.regions?.map((region: any) => 
      region.lines?.map((line: any) => line.text).join('')
    ).join('') || ''
   ...
   * 京峰验证码识别
   */
  private async recognizeWithJfbym(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
   ...
   * 图图识别
   */
  private async recognizeWithTtshitu(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
   ...
   * 自定义API识别
   */
  private async recognizeWithCustom(imageData: string | Buffer, captchaType: string): Promise<CaptchaResult> {
