          ...
          <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
            ...
            <el-form label-position="top">
              ...
              <el-form-item label="超时设置（秒）">
                <el-input-number v-model="settings.timeout" :min="0" :max="300" class="w-32" />
              </el-form-item>
        ...
        <div class="tab-content-container">
          <div class="bg-white dark:bg-gray-800 p-6 rounded-lg">
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div class="font-medium" :class="getLicenseTypeColor(licenseStatus.type)">
                      {{ licenseStatus.type }}
                    </div>
                  </div>
                  <div>
                    <span class="text-sm text-gray-500 dark:text-gray-400">状态:</span>
                    <div class="font-medium" :class="getLicenseStatusColor(licenseStatus.status)">
                  ...
                  <div v-if="licenseStatus.remainingDays !== undefined">
                    ...
                    <div
                      class="font-medium"
                      :class="licenseStatus.remainingDays < 30 ? 'text-red-500' : 'text-green-500'"
                    >
                  ...
                  <div>
                    ...
                    <div class="font-medium">
                      {{ licenseStatus.executionCount }} /
                      {{
                        licenseStatus.maxExecutions === -1 ? '无限制' : licenseStatus.maxExecutions
                      }}
                    </div>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="border border-gray-300 dark:border-gray-600 rounded-lg p-4">
                  ...
                  <el-button
                    type="primary"
                    class="w-full"
                    @click="openPurchaseLink('professional')"
                  >
                    立即购买
                  </el-button>
                </div>
            ...
            <div>
              ...
              <div class="flex space-x-4">
                <el-button :disabled="licenseStatus.type !== '免费版'" @click="startTrial">
                  开始30天试用
                </el-button>
                <el-button type="danger" @click="resetLicense"> 重置为免费版 </el-button>
              </div>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div class="space-y-2">
                  ...
                  <div>
                    ...
                    <span class="ml-2 text-gray-500 dark:text-gray-400"> 保留最近7天 </span>
                  </div>
                </div>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  const clearCache = async () => {
    try {
      await ElMessageBox.confirm('确定要清理缓存吗？', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
  ...
  const clearLogs = async () => {
    try {
      await ElMessageBox.confirm('确定要清理7天前的日志吗？', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
  ...
  const confirmShutdown = (force: boolean) =>
    doPowerAction(() => systemControl.shutdown(force), `确定要${force ? '强制' : ''}关机吗？`)
  const confirmRestart = (force: boolean) =>
    doPowerAction(() => systemControl.restart(force), `确定要${force ? '强制' : ''}重启吗？`)
  ...
  const resetSettings = async () => {
    try {
      await ElMessageBox.confirm('确定要重置所有设置吗？', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
  ...
  const resetLicense = async () => {
    try {
      await ElMessageBox.confirm('确定要重置为免费版吗？这将清除当前的许可证信息。', '重置许可证', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
