  ...
  <div class="tasks-wrapper">
    ...
    <div class="action-bar">
      ...
      <div class="button-group">
        ...
        <el-button type="success" :disabled="!selectedTasks.length" @click="handleBatchStart">
          <el-icon><VideoPlay /></el-icon>
          批量启动
        </el-button>
        <el-button type="warning" :disabled="!selectedTasks.length" @click="handleBatchStop">
        ...
        <el-table-column fixed="right" label="操作" width="300">
          <template #default="scope">
            <div class="operation-buttons">
              ...
              <el-button size="small" type="info" @click="handleSchedule(scope.row)">
                调度
              </el-button>
    ...
    <!-- 新增：任务调度对话框 -->
    <el-dialog v-model="scheduleDialogVisible" title="任务调度设置" width="700px" destroy-on-close>
      <ScheduleComponent
...
<script setup lang="ts">
        ...
        if (configResult.success) {
          ...
          const configTasks = configResult.data
            ...
            .map((config: any) => {
              ...
              console.log(
                `配置 ${config.name} (ID: ${config.id}) 转换为任务，执行次数: ${task.executionCount}`
              )
              return task
            })
          ...
          for (const task of configTasks) {
            try {
              ...
              console.log(
                `已将配置 ${task.name} (ID: ${task.id}) 转换为任务，结果:`,
                result.data.id
              )
  ...
  const filteredTasks = computed(() => {
    ...
    return tasks.value.filter(task => task.name.toLowerCase().includes(query))
  })
