  ...
  <div class="space-y-4">
    ...
    <template v-if="config.formType === 'excel'">
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center space-x-2">
          <el-input v-model="config.dataFile" placeholder="选择Excel文件" readonly class="flex-1" />
          <el-button @click="selectDataFile"> 浏览 </el-button>
        </div>
      </div>
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center space-x-2">
          <el-input v-model="config.range" placeholder="例如：A1:D10" class="flex-1" />
        </div>
      </div>
    </template>
    ...
    <template v-if="config.formType === 'web'">
      ...
      <div class="space-y-2">
        ...
        <div class="space-y-2">
          ...
          >
            <el-input v-model="field.name" placeholder="字段名称" class="w-1/3" />
            <el-input v-model="field.selector" placeholder="元素选择器" class="w-1/3" />
        ...
        <el-button class="mt-2" @click="addField"> 添加字段 </el-button>
      </div>
    </template>
    ...
    <div class="space-y-2">
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
        >等待时间（毫秒）</label
      >
      <el-input-number v-model="config.delay" :min="0" :max="10000" :step="100" class="w-full" />
    </div>
  </div>
</template>
