<template>
  ...
  >
    <div class="ai-assistant-container">
      <!-- AI 配置区域 -->
      <div class="ai-config-section" v-if="showConfig">
        <el-form :model="config" label-width="100px">
          <el-form-item label="AI 提供商">
            <el-select v-model="config.provider" placeholder="选择 AI 提供商" @change="onProviderChange">
          ...
          <el-form-item label="API URL" v-if="config.provider === 'custom'">
            <el-input
              v-model="config.apiUrl"
              placeholder="请输入自定义API地址，如：https://api.deepseek.com/v1/chat/completions"
            />
          </el-form-item>
          <el-form-item label="API Key" v-if="config.provider">
            <el-input
          ...
          <el-form-item label="模型" v-if="config.provider === 'openai'">
          ...
          <el-form-item label="模型" v-if="config.provider === 'deepseek'">
            <el-select v-model="config.model" placeholder="选择模型">
              <el-option label="DeepSeek Chat" value="deepseek-chat" />
              <el-option label="DeepSeek Coder" value="deepseek-coder" />
            </el-select>
          </el-form-item>
          <el-form-item label="模型" v-if="config.provider === 'zhipu'">
            <el-select v-model="config.model" placeholder="选择模型">
