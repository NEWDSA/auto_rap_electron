...
export class IntelligentRecorder {
   ...
   * 获取最佳选择器
   */
  private getBestSelector(target: RecordAction['target']): {
    selector: string
    selectorType: 'css' | 'xpath' | 'id' | 'class' | 'name'
  } {
    if (!target) {
    ...
    // 根据配置和目标元素情况选择最佳选择器
    if (
      this.options.elementSelectorPreference === 'css' ||
      this.options.elementSelectorPreference === 'mixed'
    ) {
    ...
    // 使用XPath作为后备
    if (
      this.options.elementSelectorPreference === 'xpath' ||
      this.options.elementSelectorPreference === 'mixed'
    ) {
