...
export class TaskScheduler extends EventEmitter {
  ...
  private async executeTask(taskId: number): Promise<boolean> {
    ...
    try {
      // 执行任务
      await this.automationController.start(task.nodes);
  ...
  private scheduleTask(task: TaskItem): void {
    ...
    // 日志记录
    this.logTaskEvent(task, `任务 "${task.name}" (ID: ${task.id}) 已调度，将在 ${new Date(nextRunTime).toLocaleString()} 执行`);
